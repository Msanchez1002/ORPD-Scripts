...
$call gams "%filepath%polar_decoupled.gms" --genPmin=%genPmin% --allon=%allon% --linelimits=%linelimits% --case="%case%" --obj=%obj% gdx=decoupledtemp.gdx lo=3 --timeperiod=%timeperiod% reslim=3600 nlp=ipopth --verbose=%verbose%
if(errorlevel ne 0, abort "Failed to find a starting point using decoupled-ACOPF!");
...
V_V.l(bus) = ic_Vm.l(bus);
V_Theta.l(bus) = ic_Va.l(bus);
V_shunt.l(bus,bus_s) = ic_shunt.l(bus,bus_s);

$if %condensed% == 'no' $include '%filepath%ic_polar%sep%calc_derived_vars.gms'
