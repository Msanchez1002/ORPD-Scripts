...
*    V_shutdown(gen,t) "(0,1) shutdown status of generator at time t"
    ...
    V_Ushunt(bus,bus_s,t)   "(0,1) Maneuver Shunt"
...
*    c_StartupShutdown(bus,bus_s,t) "Relationship of binary (start,shut,status) variables"
*    c_StartupShutdownLogic(bus,bus_s,t) "Relationship of binary (start,shut,status) variables"


    c_ManeuverTapP(i,j,c,t)   "Maneuver Tap Postive ABS"
    c_ManeuverTapN(i,j,c,t)    "Maneuver Tap Negative ABS"
    c_ManeuverShuntP(bus,bus_s,t) "Maneuver Shunt Negative ABS"
    c_ManeuverShuntN(bus,bus_s,t) "Maneuver Shunt Negative ABS"

    c_BinShunt(bus,bus_s,t)     "Binary variable Shunt"
...
c_ManeuverShuntP(bus,bus_s,t)..
    V_shunt(bus,bus_s,t) - V_shunt(bus,bus_s,t-1) =l= V_Ushunt(bus,bus_s,t) * numBswitched(bus,bus_s);
c_ManeuverShuntN(bus,bus_s,t)..
    V_shunt(bus,bus_s,t-1) - V_shunt(bus,bus_s,t) =l= V_Ushunt(bus,bus_s,t) * numBswitched(bus,bus_s);

* ABS value Tap ratio transformer maneuver
c_ManeuverTapP(i,j,c,t)$(transformer(i,j,c))..
    V_ratio(i,j,c,t) - V_ratio(i,j,c,t-1) =l= 0.2 * V_Uratio(i,j,c,t);
c_ManeuverTapN(i,j,c,t)$(transformer(i,j,c))..
    V_ratio(i,j,c,t-1) - V_ratio(i,j,c,t) =l= 0.2 * V_Uratio(i,j,c,t);

* Binary Shunt and Tap variables
c_BinShunt(bus,bus_s,t)..
    V_Ushunt(bus,bus_s,t) * (1 - V_Ushunt(bus,bus_s,t)) =e= 0;
c_BinTap(i,j,c,t)..
...
V_ratio.fx(i,j,c,t)$(not transformer(i,j,c)) = ratio(i,j,c,t);
V_ratio.l(i,j,c,t) = ratio(i,j,c,t);

* Derived variables
V_LineP.l(i,j,c,t)$branchstatus(i,j,c,t) =  (g(i,j,c) * sqr(V_V.l(i,t)) / sqr(V_ratio(i,j,c,t)))
            - (V_V.l(i,t) * V_V.l(j,t) / V_ratio(i,j,c,t)) *
 ...
 V_LineQ.l(i,j,c,t)$branchstatus(i,j,c,t) = - (sqr(V_V.l(i,t)) * (b(i,j,c) + bc(i,j,c)/2) / sqr(V_ratio(i,j,c,t)))
            - (V_V.l(i,t) * V_V.l(j,t) / V_ratio(i,j,c,t)) *
